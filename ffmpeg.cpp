...
int InitInput(const char * Filename,AVFormatContext ** iframe_c,AVInputFormat** iinputframe)
{
    int i = 0;
...
AVDictionary *options;
    if (avformat_find_stream_info(*iframe_c, &options) < 0)
    {
...
int InitOutput()
{
    ...
    int ret = avformat_alloc_output_context2(&oc, NULL, "flv", OUTPUTURL);
    if (!oc)
    {
        return getchar();
    }
    ...
    {
        if (ret = avio_open(&oc->pb, OUTPUTURL, AVIO_FLAG_WRITE) < 0)
        {
    ...
    qDebug() << "-------------------------video_st" <<video_st;

    if (ret = OpenCodec(video_st,video_codec) < 0)   //打开视频编码器
    {
        printf("can not open video codec\n");
        return getchar();
    }
    qDebug() << "-------------------------OpenCodec" <<ret;
    ...
    if (ret = avformat_write_header(oc, NULL))
    {
